import { TEL_HREF } from "../../config/site";
...
function ProductSkeleton() {
  return (
    <div className="overflow-hidden rounded-3xl border border-ocean-100 bg-white shadow-card">
      <div className="aspect-4/3 w-full animate-pulse bg-ocean-100" />
      <div className="space-y-3 p-7">
        <div className="h-6 w-1/2 animate-pulse rounded bg-ocean-100" />
        <div className="h-4 w-full animate-pulse rounded bg-ocean-100" />
        <div className="h-4 w-2/3 animate-pulse rounded bg-ocean-100" />
      </div>
    </div>
  );
}
  ...
  return (
    <section id="san-pham" className="scroll-mt-20 py-20 sm:py-28">
      <Container>
        ...
        {error ? (
          <p className="mt-14 text-center text-coral-600">
              ...
              : products.map((p, i) => (
                  ...
                  >
                    ...
                    {p.img && (
                      <div className="relative aspect-4/3 overflow-hidden">
                        <SmartImage
                          ...
                          fallbackClass={`bg-linear-to-br ${p.tint}`}
                    ...
                    <div className="relative flex flex-1 flex-col p-7">
                      {/* tint glow */}
                      <div
                        className={`pointer-events-none absolute -right-10 -top-10 h-32 w-32 rounded-full bg-linear-to-br ${p.tint} opacity-70 blur-2xl transition-transform duration-500 group-hover:scale-150`}
                      />
                      <div className="relative flex flex-1 flex-col">
                        {/* Ô icon — chỉ hiện khi chưa có ảnh */}
                        {!p.img && (
                          <div
                            className={`grid h-16 w-16 place-items-center rounded-2xl bg-linear-to-br ${p.tint} text-4xl shadow-inner`}
                          >
                            <span aria-hidden="true">{p.emoji}</span>
                          </div>
                        )}
